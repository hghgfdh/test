...
# --- Configuration ---
target = "59:58"  # Target time in "MM:SS" format
...
col_r = Colors.RED
col_c = Colors.CYAN
col_m = Colors.MAGENTA

# --- Global Variables ---
valid_cookies = []  # Stores valid cookies
results = []  # Stores results for each request (thread-safe list)
lock = threading.Lock()  # Thread-safe lock for results
request_queue = Queue()  # Queue for real-time printing
...
def send_request(session, cookie_value, request_time, index):
    device_id = generate_device_id()
    ...
    short_cookie = f"{cookie_value[:10]}..." if len(cookie_value) > 10 else cookie_value
    print(col_m + f"[Request #{index}] Sent at {request_time.strftime('%H:%M:%S.%f')[:-3]} | Cookie: {short_cookie} | Device ID: {device_id[:8]}..." + Colors.RESET)
    ...
    try:
        ...
        if response:
            ...
            with lock:
                results.append({
                    "cookie": short_cookie,
                    "time": request_time.strftime('%H:%M:%S.%f')[:-3],
            ...
            print(col_g + f"[Response #{index}] Received at {datetime.now().strftime('%H:%M:%S.%f')[:-3]} | Cookie: {short_cookie} | Code: {code} | Data: {data}" + Colors.RESET)
        else:
            with lock:
                results.append({
                    "cookie": short_cookie,
            ...
            print(col_r + f"[Response #{index}] No response received for Cookie: {short_cookie}" + Colors.RESET)
    except Exception as e:
        with lock:
            results.append({
                "cookie": short_cookie,
        ...
        print(col_r + f"[Response #{index}] Error for Cookie: {short_cookie} | Error: {str(e)}" + Colors.RESET)
...
def wait_until_feed_time():
    ...
    while True:
        ...
        if now.minute == target_min and now.second == target_sec:
            ...
            if now >= feed_time:
                print(col_g + f"\n[Feed Time Reached]: Starting requests at {now.strftime('%H:%M:%S.%f')[:-3]}" + Colors.RESET)
                return
        time.sleep(0.1)
...
def main_loop(session):
    ...
    while datetime.now() < stop_time:
        ...
        threading.Thread(
            target=send_request,
            args=(session, cookie, request_time, index + 1),
            daemon=True
        ).start()

        index = (index + 1) % len(valid_cookies)  # Loop back to the first cookie
        time.sleep(request_interval / 1000)  # Convert ms to seconds

# --- Print Results ---
def print_results():
    print("\n" + col_c + "=== Final Results ===" + Colors.RESET)
    if not results:
    ...
    print(f"{col_g}[Total Requests]: {len(results)}{Colors.RESET}")
    print(f"{col_g}[Successful]: {success_count}{Colors.RESET}")
    print(f"{col_r}[Failed]: {failed_count}{Colors.RESET}\n")

    for i, result in enumerate(results, 1):
        print(
            f"{i}. {col_b}[Cookie]: {result['cookie']}{Colors.RESET} "
            f"{col_g}[Time]: {result['time']}{Colors.RESET} "
            f"{col_y}[Device ID]: {result['device_id']}{Colors.RESET} "
            f"{col_c}[Status]: {result['status']}{Colors.RESET}"
        )
        if "code" in result:
            print(f"   {col_y}[Code]: {result['code']}{Colors.RESET}")
        if "error" in result:
            print(f"   {col_r}[Error]: {result['error']}{Colors.RESET}")
        if "data" in result:
            print(f"   {col_y}[Data]: {result['data']}{Colors.RESET}")
